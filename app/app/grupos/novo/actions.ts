...
export async function createGroup(
    ...
    if (errorParticipants) {
        return {
            success: false,
            message: "Ocorreu ume rro ao adicionar os participantes ao grupo. Por favor tente novamente!"
        };
    }

    const drawnParticipants = drawGroup(createdParticipants);

    const { error: errorDraw } = await supabase.from("participants")
...
async function sendEmailToParticipants(participants: Participant[], groupName: string) {
    ...
    try {
        ...
        return { error: "Ocorreu um erro ao enviar o emails."};
    }
}